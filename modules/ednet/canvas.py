...
class Canvas:
    ...
    @staticmethod
    def Init():
        if (Canvas._init_run != True):
            Canvas._canvas_enabled = AppSettings.GetValue('canvas_import_enabled', False)
            Canvas._canvas_dev_key = AppSettings.GetValue('canvas_dev_key', '')
    ...
    def Connect():
        ...
        if (Canvas._canvas_server_url == "" or Canvas._canvas_dev_key == ""):
            Canvas._errors.append("<b>Canvas not properly configured!</b>")
            return False # Canvas not configured
        
        if (Canvas._connect_run != True):
            # Get the admin user to see if things are working
            Canvas._admin_user = Canvas.APICall(Canvas._canvas_server_url, Canvas._canvas_dev_key, "/api/v1/accounts/self")
            if (Canvas._admin_user == None or Canvas._admin_user.get('id', None) == None):
                # Invalid user
    ...
    def CreateUser(user_name, password, first_name, last_name, email):
        ...
        canvas_user = Canvas.APICall(Canvas._canvas_server_url, Canvas._canvas_dev_key, "/api/v1/users/sis_user_id:" + user_name + "/profile")
        if ('sis_user_id' not in canvas_user):
            ...
            r = Canvas.APICall(Canvas._canvas_server_url, Canvas._canvas_dev_key, "/api/v1/accounts/self/users", method="POST", params=p)
            ...
            if ('sis_user_id' not in canvas_user):
                ...
                h = dict()
                r = Canvas.APICall(Canvas._canvas_server_url, Canvas._canvas_dev_key, "/api/v1/accounts/" + str(Canvas._admin_user["id"]) + "/sis_imports", method="POST", params=p, files=files, headers=h)
                ...
                if ('id' in r):
                    while (tries < max_tries):
                        time.sleep(0.5)
                        progress = Canvas.APICall(Canvas._canvas_server_url, Canvas._canvas_dev_key, "/api/v1/accounts/" + str(Canvas._admin_user["id"]) + "/sis_imports/" + str(r["id"]))
        ...
        p["user[short_name]"] = last_name + ", " + first_name + " (" + user_name + ")"
        p["user[sortable_name]"] = last_name + ", " + first_name + " (" + user_name + ")"
        canvas_user = Canvas.APICall(Canvas._canvas_server_url, Canvas._canvas_dev_key, "/api/v1/users/sis_user_id:" + user_name, method="PUT", params=p)
    ...
    def SetPassword(user_name, new_password):
        ...
        account_list = Canvas.APICall(Canvas._canvas_server_url, Canvas._canvas_dev_key, "/api/v1/users/sis_user_id:" + user_name + "/logins")
        for account in account_list:
            if ("account_id" in account):
                ...
                Canvas.APICall(Canvas._canvas_server_url, Canvas._canvas_dev_key, "/api/v1/accounts/" + str(account["account_id"]) + "/logins/" + str(account["id"]), method="PUT", params=q)
    ...
    def GetCurrentClasses(user_name):
        ...
        current_enrollment = Canvas.APICall(Canvas._canvas_server_url, Canvas._canvas_dev_key, "/api/v1/users/sis_user_id:" + user_name + "/enrollments")
    ...
    def CompleteAllClasses(user_name, faculty_delete=False):
        ...
        for enrolled_class in current_enrollment:
            if ('type' in enrolled_class and enrolled_class['type'] == "StudentEnrollment"):
                ...
                Canvas.APICall(Canvas._canvas_server_url, Canvas._canvas_dev_key, api, method="DELETE", params=q)
    ...
    def CreateCourse(course_name):
        ...
        # Get the course info
        course_info = Canvas.APICall(Canvas._canvas_server_url, Canvas._canvas_dev_key, "/api/v1/accounts/" + str(Canvas._admin_user["id"]) + "/courses/sis_course_id:" + str(course_name))
        if ('account_id' not in course_info):
            ...
            if (Canvas._canvas_auto_create_courses != True):
                ...
                q["offer"] = "true"
                course_info = Canvas.APICall(Canvas._canvas_server_url, Canvas._canvas_dev_key, "/api/v1/accounts/" + str(Canvas._admin_user["id"]) + "/courses", method="POST", params=q);
    ...
    def EnrollStudent(canvas_user, enroll_class):
        ...
        # Get the course info
        course_info = Canvas.APICall(Canvas._canvas_server_url, Canvas._canvas_dev_key, "/api/v1/accounts/" + str(Canvas._admin_user["id"]) + "/courses/sis_course_id:" + str(enroll_class))
        if ('account_id' not in course_info):
            # Course doesn't exist?!
        ...
        if ('id' in canvas_user):
            ...
            res = Canvas.APICall(Canvas._canvas_server_url, Canvas._canvas_dev_key, "/api/v1/courses/" + str(course_info["id"]) + "/enrollments", method="POST", params=q)
