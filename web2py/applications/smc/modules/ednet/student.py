...
class Student:
    ...
    def CreateW2PyAccounts(sheet_name, override_password=False, override_current_quota=False):
        ...
        for row in rows:
            ...
            W2Py.CreateW2PStudentUser(user_name, password, user_email, first_name, last_name, user_ad_quota, user_canvas_quota, row)
        return count
    ...
    def GetEmail(student_id):
        # Default -Prepend S and append @domain.com (e.g. s1888182@correctionsed.com)
        user_name = Student.GetUsername(student_id)
        pattern = AppSettings.GetValue('student_email_pattern', 's<user_name>@correctionsed.com')
        ret = pattern.replace('<user_id>', student_id)
    ...
    def GetHomeDirectory(student_id):
        ...
        ret = ret.replace('<user_id>', student_id)
    ...
    def ProcessADStudent():
        ...
        rows = db(db.student_import_queue.id==db.student_ad_import_queue.student_import_queue).select(orderby=db.student_import_queue.account_enabled|db.student_import_queue.student_name,limitby=(0,1))
        ...
        for row in rows:
            ...
            student_password = Student.GetPassword(row.student_import_queue.user_id, row.student_import_queue.student_password)
            ...
            student_dn = Student.GetAD_DN(student_user_name, row.student_import_queue.program) #AD.GetDN(student_user_name, ad_student_cn)
